...
function getNavItems(role: User['role'], badgeCounts: { orders: number; deposits: number; withdrawals: number }): NavItem[] {
  const BUYER: NavItem[] = [
  ...
  const ADMIN: NavItem[] = [
    ...
    { icon: '📦', label: 'Commandes', href: '/admin/orders', badge: 'pendingOrders' },
    { icon: '💰', label: 'Dépôts', href: '/admin/deposits', badge: 'pendingDeposits' },
    { icon: '💸', label: 'Retraits', href: '/admin/withdrawals', badge: 'pendingWithdrawals' },
    ...
    <>
      ...
      >
        ...
        <div className="flex h-16 items-center gap-2 border-b border-[#1e1e1e] px-4">
          <Link href={role === 'ADMIN' ? '/admin' : '/dashboard'} className="text-xl font-bold text-[#6366f1]">
            Tunixo
          </Link>
          {role === 'ADMIN' && (
        ...
        >
          {navItems.map((item, idx) => {
            ...
            const { icon, label, href, badge } = item as Exclude<NavItem, { divider: true }>
            const count =
              badge === 'pendingOrders'
                ? badgeCounts.orders
        ...
        {showWalletSection && (
          <div className="mx-3 mb-3 rounded-xl border border-white/10 bg-white/5 p-3">
            ...
            <p className="mb-2 font-bold text-white">{formatTND(balance)} TND</p>
            <Link
              href="/wallet"
